...
//constantly running functions
      ...
      function redrawLines(){
        ...
        if(clearedAndDrawnOver == true)return;
        preDraw();
...
//checks if mouse unclicks on top of the instructions while drawing
let mouseUpOnInstruct =false;
function mouseUpInstructions(){
   $(".instructions").on("mouseup", ()=>{
      mouseUpOnInstruct = true;
  })
}

//checks if mouse is outside of canvas and returns where it is in relation to canvas
...
canvas.addEventListener('mouseup', () =>{
   ...
   mouseUpInstructions();
   if(mouseUpOnInstruct != false)return;
...
window.addEventListener('mouseup', ()=>{
 ...
 const yMouse = event.pageY - offsetTop;
  if(intentional == true)return;
  mouseUpInstructions();
    if(mouseUpOnInstruct ==true){
    isItDrawing = false;
    currentPoint = {color:lineColor, startX: xMouse, startY: yMouse, mode: "end", percentY: calculatePercent(yMouse)};
    allPoints46.push(currentLine);
    firstLine = [].concat(...allPoints46);
    currentLine = [];
    mouseUpOnInstruct = false;
    return;
   };
  console.log(intentional)
   console.log(whereIsMouse());
    if(whereIsMouse()== 'top'){
